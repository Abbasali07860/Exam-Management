...
<div class="container mx-auto px-4 p-12 bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl shadow-2xl max-w-7xl">
    <h2 class="text-3xl font-extrabold text-gray-900 mb-8">Edit Class</h2>

    <form method="POST" action="{{ route('admin.classrooms.update', $classroom->id) }}" class="bg-white p-8 rounded-2xl shadow-md border border-gray-200">
        @csrf
        @method('PUT')
                    ...
                    @foreach($subjects as $subject)
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="subject_ids[]" value="{{ $subject->id }}"
                                   ...
                                   {{ $classroom->subjects->contains($subject->id) || in_array($subject->id, old('subject_ids', [])) ? 'checked' : '' }}>
