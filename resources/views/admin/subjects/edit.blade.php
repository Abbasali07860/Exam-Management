...
<div class="container mx-auto px-4 p-12 bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl shadow-2xl max-w-7xl">
    <h2 class="text-3xl font-extrabold text-gray-900 mb-8">Edit Subject</h2>

    <form method="POST" action="{{ route('admin.subjects.update', $subject->id) }}" class="bg-white p-8 rounded-2xl shadow-md border border-gray-200">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <select name="classroom_id" id="classroom_id"
                ...
                @foreach($classrooms as $classroom)
                    <option value="{{ $classroom->id }}" {{ old('classroom_id', $subject->classrooms->first()?->id) == $classroom->id ? 'selected' : '' }}>
                        {{ $classroom->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-6">
            ...
            <select name="teacher_id" id="teacher_id"
                ...
                @foreach($teachers as $teacher)
                    <option value="{{ $teacher->id }}" {{ old('teacher_id', $subject->teachers->first()?->id) == $teacher->id ? 'selected' : '' }}>
                        {{ $teacher->name }}
                    </option>
                @endforeach
            </select>
