...
<div class="container mx-auto px-4 p-12 bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl shadow-2xl max-w-8xl">
    <div class="flex justify-between items-center mb-8">
        <h2 class="text-3xl font-extrabold bg-gradient-to-r from-indigo-600 to-green-600 bg-clip-text text-transparent">Exam Management</h2>
        <div class="flex gap-4">
            <a href="{{ route('admin.exams.create') }}"
               class="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-6 py-3 rounded-xl text-sm font-semibold shadow-lg hover:scale-105 hover:shadow-xl transition-all duration-300">
                Add New Exam
            </a>
            <a href="#" id="open-status-modal"
               class="bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white px-6 py-3 rounded-xl text-sm font-semibold shadow-lg hover:scale-105 hover:shadow-xl transition-all duration-300">
                Change Status
            </a>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-base">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="even:bg-gray-50 hover:bg-indigo-50 transition-all duration-300 transform hover:scale-[1.01]">
                        ...
                        <td class="px-6 py-4 text-gray-700">{{ $exam->start_date->format('Y-m-d H:i') }}</td>
                        <td class="px-6 py-4 text-gray-700">{{ $exam->end_date->format('Y-m-d H:i') }}</td>
                        ...
                        <td class="px-6 py-4 text-gray-700">
                            {{ $exam->users->pluck('name')->implode(', ') ?: 'None' }}
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <a href="{{ route('admin.exams.edit', $exam->id) }}"
                                class="inline-flex items-center bg-gradient-to-r from-gray-200 to-gray-300 text-gray-800 px-4 py-2 rounded-xl font-semibold hover:from-gray-300 hover:to-gray-400 hover:scale-105 transition-all duration-300">
                                    Edit
                                </a>
                                <form method="POST" action="{{ route('admin.exams.destroy', $exam->id) }}" class="inline-block">
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="inline-flex items-center bg-gradient-to-r from-red-500 to-red-600 text-white px-4 py-2 rounded-xl font-semibold hover:from-red-600 hover:to-red-700 hover:scale-105 transition-all duration-300"
